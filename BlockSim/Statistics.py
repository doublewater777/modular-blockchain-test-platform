...
class Statistics:
    ...
    def blocks_results(t):
        ...
        for b in c.global_chain:
            ...
            transCreateTime += b.transCreateTime
            transExeTime += b.transExeTime
            BlockExeTime += b.BlockExeTime
        ...
        Statistics.blockData = [Statistics.totalBlocks, Statistics.mainBlocks,  Statistics.uncleBlocks, Statistics.uncleRate,
                                Statistics.staleBlocks, Statistics.staleRate, trans, transCreateTime, transExeTime, BlockExeTime, t]
    ...
    def global_chain():
        if p.model == 0 or p.model == 1:
            for i in c.global_chain:
                block= [i.depth, i.id, i.previous, i.timestamp, i.miner, len(i.transactions), i.size,i.transCreateTime,i.transExeTime,i.BlockExeTime]
    ...
    def print_to_excel(fname):
        ...
        df2.columns = ['Total Blocks', 'Main Blocks', 'Uncle blocks', 'Uncle Rate', 'Stale Blocks', 'Stale Rate',
                       '# transactions', 'transactions createTime', 'transactions exeTime', 'BlockExeTime', 'performance']
        ...
        if p.model == 2:
            ...
            df4.columns = ['Block Depth', 'Block ID', 'Previous Block', 'Block Timestamp',
                           'Miner ID', '# transactions', 'Block Size', 'TcreateTime', 'TexeTime', 'exeTime']
