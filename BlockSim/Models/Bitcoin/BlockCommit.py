...
from Utils.HashUtil import merkle_root, getHash
import json,time
        ...
        if blockPrev == miner.last_block().id:
            ...
            if p.hasTrans:
                if p.Ttechnique == "Light":
                    blockTrans, blockSize,createTime,exeTime = LT.execute_transactions()
                elif p.Ttechnique == "Full":
                    blockTrans, blockSize = FT.execute_transactions(
                        miner, eventTime)

                event.block.transactions = blockTrans
                event.block.transCreateTime = createTime
                event.block.transExeTime=exeTime
                event.block.usedgas = blockSize
                event.block.mkRoot = merkle_root(
                    [t.id for t in blockTrans], p.hashAlg)
                x = json.dumps([[i.id for i in event.block.transactions], event.block.previous],
                               sort_keys=True)
                event.block.id = getHash(x, p.hashAlg)
                t2 = time.time()
                event.block.BlockExeTime = t2-t1
