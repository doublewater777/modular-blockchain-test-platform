...
class LightTransaction():
    ...
    def create_transactions1():
        ...
        for i in range(Psize):
            ...
            tx.id = HashUtil.getHash(hashdata, p.hashAlg)
            
            tx.signature = HashUtil.sign2(
                tx.sender, hashdata, p.signAlg)
    ...
    def execute_transactions1():
        ...
        while count < len(pool):
            ...
            ver = HashUtil.verify2(
                    txObj.sender, hashdata, txObj.signature, p.signAlg)
    ...
    def create_transactions():
        api.genTx(hashAlg=p.hashAlg,signAlg=p.signAlg)
    def execute_transactions():
        res = api.exeTx(signAlg=p.signAlg)
        transaction_dict, size,create_time,exe_time = (res)
        transactions =[Transaction(p["id"], p["timestamp"],p["sender"],p["to"],p["value"],p["size"],p["fee"]) for p in transaction_dict]
        return transactions,size,create_time,exe_time
