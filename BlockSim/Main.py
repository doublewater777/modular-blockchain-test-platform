...
from Utils.HashUtil import genKeys
import json
import time
if p.model == 3:
...
def main(hashAlg, signAlg):
    p.hashAlg = hashAlg
    p.signAlg = signAlg
    eventTime = 0
    for i in range(p.Runs):
        t1 = time.time()
        genKeys(signAlg=p.signAlg)
        t2 = time.time()
        print('生成密钥所需时间', t2-t1)
        ...
        while not Queue.isEmpty() and clock <= p.simTime:
            ...
            print(clock, 'clock')
            print('eventType', next_event.type, 'id', next_event.node)
        ...
        result = Statistics.blocksResults
        if p.model == 3:
